...
- (YMBankCardModel *)getPayTypeModel
{
    ...
    }else if ([self getUseType]==9){//请选择支付方式
        YMBankCardModel *bankCardModel = [[YMBankCardModel alloc] init];
        bankCardModel.userTypeNum = 9;
        return bankCardModel;
    }
    return nil;
}
...
- (YMBankCardModel *)getCurrentPayTypeModel:(NSString *)paySign
{
    if ([self getUseType] == 1) {//银行卡
        YMBankCardModel *bankCardModel = [[YMBankCardModel alloc] init];
        for (YMBankCardModel *model in [self getBankCardListArray]) {
            if ([model.getPaySignStr isEqualToString:paySign]) {
                bankCardModel = model;
                return bankCardModel;
            }
        }
