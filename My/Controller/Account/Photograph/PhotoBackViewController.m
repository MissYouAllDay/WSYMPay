//
//  PhotoBackViewController.m
//  WSYMPay
//
//  Created by 赢联 on 16/9/23.
...
@interface PhotoBackViewController ()
...
@implementation PhotoBackViewController
...
- (LPCarema *)caremaView
{
    if (_caremaView == nil) {
        ...
        UIImageView *imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"backimg"]];
        imageView.frame = CGRectMake(53/375.0*SCREENWIDTH, 160/667.0*SCREENHEIGHT, 70/375.0*SCREENWIDTH, 75/375.0*SCREENWIDTH);
        ...
        titleLab.text = @"将身份证反面置于此区域，拍摄身份证";
...
-(void)backBtnTouchUp:(UIButton *)sender
{
    ...
    for(UIViewController *temVC in temArray)
        {
        if ([temVC isKindOfClass:[IDVerificationViewController class]])
        {
...
- (void)finishBtn:(UIButton *)button{    
    UploadIDCardViewController *udc = [[UploadIDCardViewController alloc]init];
    udc.frontImage = self.frontImage;
    udc.backImage = self.imageView.image;
    [self.navigationController pushViewController:udc animated:YES];
    
    [self dissmissCurrentViewController:2];
}
